...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('ablation_model_train.log'), logging.StreamHandler()]
)
...
# 移除混淆特征分支的基线模型
# ------------------------------
class AblationBaselineDetector(InjectionAttackDetector):
    ...
    def forward(self, input_ids):
        ...
        # obfuscation_features = torch.cat([hex_pool, unicode_pool], dim=1)

        # 主干CNN分支完全不变
        embeddings_transposed = embeddings.transpose(1, 2)
        conv1_out = self.relu(self.pool(self.conv1(embeddings_transposed))).transpose(1, 2)
        conv2_out = self.relu(self.pool(self.conv2(embeddings_transposed))).transpose(1, 2)
        conv3_out = self.relu(self.pool(self.conv3(embeddings_transposed))).transpose(1, 2)
        cnn_output = torch.cat([conv1_out, conv2_out, conv3_out], dim=2)
...
class EnhancedInjectionDetector(InjectionAttackDetector):
    ...
    def forward(self, input_ids):
        ...
        combined_features = torch.cat([attention_output, obfuscation_features], dim=1)
...
# ==================== 6. 测试函数 ====================
def batch_test_model(trainer, test_loader, threshold=0.5, save_metrics=True, model_name="model"):
    trainer.model.eval()
    all_preds = []
    all_labels = []
    all_probs = []
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"【{model_name}】批量测试结果（阈值 = {threshold:.2f}）")
    logger.info("=" * 60)
...
# ==================== 7. 训练/测试主函数（支持消融实验） ====================
def train_model(model_type='complete', data_dir=Config.CSIC_DATA_DIR, max_len=Config.MAX_LEN, epochs=Config.EPOCHS, device=Config.DEVICE):
    ...
    logger.info(f"\n初始化模型：{'【完整模型】EnhancedInjectionDetector' if model_type == 'complete' else '【消融基线模型】AblationBaselineDetector'}...")
    if model_type == 'complete':
        ...
        save_path = 'best_csic_model.pth'
    else:
        model = AblationBaselineDetector(
            vocab_size=vocab_size,
            embed_dim=Config.EMBED_DIM,
            max_len=max_len,
            cnn_channels=Config.CNN_CHANNELS,
        ...
        save_path = 'best_ablation_model.pth'
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"验证损失: {test_loss:.4f} | 验证准确率: {test_acc:.2f}%")
        logger.info(f"当前最优验证准确率: {best_test_acc:.2f}% | 早停计数: {early_stop_count}/{Config.EARLY_STOP_PATIENCE}")
        logger.info("-" * 60)
    ...
    logger.info(f"\n训练完成！最优模型已保存为 {save_path}（最优验证准确率：{best_test_acc:.2f}%）")

def load_trained_model(model_path, device=Config.DEVICE):
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"模型文件 {model_path} 不存在！")
    
    checkpoint = torch.load(model_path, map_location=torch.device(device))
    model_type = checkpoint.get('model_type', 'complete')
...
def test_model(model_path, data_dir=Config.CSIC_DATA_DIR, max_len=Config.MAX_LEN, device=Config.DEVICE):
    checkpoint = torch.load(model_path, map_location=torch.device(device))
    word2idx = checkpoint['word2idx']
    vocab_size = checkpoint['vocab_size']
    model_type = checkpoint.get('model_type', 'complete')
    model_name = '完整模型' if model_type == 'complete' else '消融基线模型'
    ...
    logger.info(f"\n加载训练好的模型：{model_name}...")
    trainer, _ = load_trained_model(model_path, device=device)
    
    thresholds = [0.5, 0.6, 0.7, 0.8, 0.9]
    for th in thresholds:
        batch_test_model(trainer, test_loader, threshold=th, save_metrics=False, model_name=model_name)

# ==================== 8. 主程序入口 ====================
if __name__ == "__main__":
    print("=" * 60)
    print("基于深度学习的注入攻击流量检测系统（含消融实验）")
    print("=" * 60)
    print("请选择操作模式：")
    print("1. 仅训练【完整模型】")
    print("2. 仅测试【完整模型】")
    print("3. 训练+测试【完整模型】")
    print("4. 【消融实验】仅训练【移除混淆分支的基线模型】")
    print("5. 【消融实验】仅测试【移除混淆分支的基线模型】")
    print("=" * 60)
    ...
    if choice == 1:
        logger.info("\n=== 开始执行：仅训练完整模型 ===")
        train_model(model_type='complete')
    elif choice == 2:
        logger.info("\n=== 开始执行：仅测试完整模型 ===")
        test_model(model_path='best_csic_model.pth')
    elif choice == 3:
        logger.info("\n=== 开始执行：训练+测试完整模型 ===")
        train_model(model_type='complete')
        test_model(model_path='best_csic_model.pth')
        ...
        logger.info("\n=== 开始执行：消融实验 - 训练基线模型 ===")
        train_model(model_type='ablation')
    elif choice == 5:
        logger.info("\n=== 开始执行：消融实验 - 测试基线模型 ===")
        test_model(model_path='best_ablation_model.pth')
